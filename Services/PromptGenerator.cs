...
namespace Services
{
    public class PromptGenerator(IOpenAIClient openAIClient, ILogger<DocumentParse> logger) : IPromptGenerator
    {
        ...
        public async Task ExecuteChain()
        {
            ...
            {
                Role = "You are a highly experienced senior C# .NET Core backend developer and software architect with a deep understanding of dependency injection, clean architecture, and modular software design.\r\n\r\nYou are tasked with migrating a Python-based skill normalization script into a production-ready C# .NET Core console application. You are comfortable working with JSON files, data cleaning logic, and categorization trees. You know how to properly abstract services using interfaces, register them using the Microsoft.Extensions.DependencyInjection DI container, and split responsibilities across classes and layers.\r\n\r\nYou are fluent in both Python and C#, allowing you to understand the original logic deeply and then restructure it following modern .NET practices. You are focused on SOLID principles, unit-testable code, logging using ILogger<T>, and external configuration with appsettings.json.\r\n\r\nYou deliver modular, testable, maintainable C# code that adheres to professional software engineering practices.\r\n\r\nYour output must always reflect your experience in enterprise-level .NET development with a strong emphasis on code quality, architecture, and extensibility.",
                Context = $"You have been given a Python script that performs skill normalization on semi-structured job descriptions. The script reads a JSON file with job postings, extracts relevant skill terms, cleans and normalizes them, groups similar skills using string similarity, assigns categories based on a hierarchical structure, and finally exports multiple JSON and text files with grouped and categorized results.\r\n\r\nThis script is currently written in a procedural, monolithic style and lacks modularity, testability, and separation of concerns.\r\n\r\nThe target is to rewrite this logic in C# using .NET Core as a modular console application that leverages dependency injection, proper service architecture, and clean code practices.\r\n\r\nThe resulting C# code should be suitable for integration into a larger system or service pipeline, and must include logging, configuration management, and testable interfaces for core logic (e.g., skill extraction, grouping, normalization, classification, and output writing).\r\n\r\nPerformance and clarity are important, but maintainability and clean architecture are the top priorities.\r\n",
                Task = "Your task is to rewrite the provided Python script in idiomatic, production-grade C# using .NET Core (version 7 or 8 preferred). The script should be refactored into a console application structured around dependency injection and clean architecture principles.\r\n\r\nYou must:\r\n\r\n1. Identify and extract core responsibilities from the Python script and map them into appropriate C# services or classes (e.g., skill extractor, skill normalizer, category resolver, file reader/writer, logger).\r\n2. Create C# interfaces and implementations for each logical service, ensuring that all dependencies are injected via constructor injection.\r\n3. Set up a dependency injection container using `Microsoft.Extensions.DependencyInjection`.\r\n4. Use `ILogger<T>` for logging, and `IConfiguration` to load paths and constants from `appsettings.json`.\r\n5. Use asynchronous file I/O where appropriate.\r\n6. Ensure the logic for normalization, grouping, and classification is clean, reusable, and easily testable.\r\n7. Implement a Program.cs or entry point that orchestrates the services and triggers the normalization process.\r\n\r\nDo not attempt a line-by-line translation — instead, translate the **functionality** into idiomatic, layered, object-oriented C#.\r\n\r\nComment the code where appropriate, especially when transforming complex logic.\r\n",
                Format = "Your response must include the following:\r\n\r\n1. A complete C# .NET interface and implementation following clean architecture and using dependency injection.\r\n2. For each key functional block from the original Python script, create an interface (e.g., ISkillExtractor, ISkillNormalizer, ICategoryResolver, IFileHandler, etc.) and corresponding class that implements it.\r\n3. The main coordination logic should reside in a class similar to `DocumentCoordinator.cs` and should implement a corresponding interface (`IDocumentCoordinator.cs` style).\r\n4. Register all dependencies using `IServiceCollection` in `Program.cs`.\r\n5. Use `ILogger<T>` for logging, `IConfiguration` for settings like file paths and thresholds (e.g., similarity, minimum group size).\r\n6. Group code into clear folders like: `/Services`, `/Interfaces`, `/Models`, and `/Helpers`.\r\n7. Structure the code so it is ready for unit testing — no static classes or hard-coded paths.\r\n8. Do not include placeholder logic. Use complete C# logic for reading JSON, processing data, grouping, writing output.\r\n9. Provide full source code blocks for key classes and the entry point (Program.cs).\r\n10. Use async/await where applicable.\r\n\r\nRespond with the full C# source code in separate markdown code blocks for each file, properly labeled (e.g., `Program.cs`, `SkillExtractor.cs`, `ISkillExtractor.cs`, etc.).",
                MaxLength = maxLengh
            };
