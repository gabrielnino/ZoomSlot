...
namespace Services
{
    public class PromptGenerator(IOpenAIClient openAIClient, ILogger<DocumentParse> logger) : IPromptGenerator
    {
        ...
        public async Task ExecuteChain()
        {
            ...
            {
                Role = "As a Job-Curriculum Fit Analyst, I specialize in developing AI-driven solutions that accurately measure the alignment between a candidate’s resume and job descriptions. Leveraging Natural Language Processing (NLP) and machine learning, I design algorithms that parse skills, experience, and qualifications to generate a precise fit percentage score, helping recruiters streamline hiring and job seekers optimize their applications. My expertise includes text analysis, semantic matching, and predictive modeling, supported by tools like Python, spaCy, and Scikit-learn. With a background in data science and HR technology, I bridge the gap between technical precision and recruitment efficiency, ensuring that matching systems are both accurate and actionable. Whether integrating with ATS platforms or refining scoring models, my goal is to enhance decision-making with data-driven insights.",
                Context = $"You are working with an existing in software company with follwoing job offers and " +
                $"" +
                $"" +
                $"",
                Task = "For the following files: index.html, app.js, and styles.css, use processed_parse_jobs.json as the input file and modify the app to also load resume.json, implementing a percentage-based gauge to calculate the accuracy between the resume and job offer, similar to fortunes-graphic-dark.png.",
                Format = "output file:" +
                "- index.html" +
                "- styles.css" +
                "- app.js",
